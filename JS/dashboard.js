/* ============================================================
   SpendBuddy Dashboard Script - MATCHING EXPENSE.JS DISPLAY
...
function getPaymentTypeName(paymentId) {
  ...
  const paymentType = dashboardState.paymentTypes.find((p) => p.id == paymentId);
...
/* --------------------------
   Display Recent Expenses - MATCHING EXPENSE.JS STYLE
-------------------------- */
    ...
    .map((exp) => `
      <div class="expense-item">
        <div class="expense-info">
          ...
          <div class="expense-details">
            ...
            <div class="expense-meta">
              <span>📅 ${CONFIG.utils.formatDate(exp.expenseDate)}</span>
        ...
        <div class="expense-amount">
          <div>${CONFIG.utils.formatCurrency(exp.expenseAmount || 0)}</div>
          <div class="expense-actions">
            <button class="btn btn-primary" onclick="openExpenseDetailModal('${exp.id}')" style="padding: 8px 12px; font-size: 0.85rem;">View</button>
...
function openEditExpenseModal(expenseId) {
  ...
  const expense = dashboardState.expenses.find(exp => String(exp.id) === String(expenseId));
  ...
  console.log("📋 Available categories:", dashboardState.categories);
  console.log("💳 Available payment types:", dashboardState.paymentTypes);
  
  // Store the ID we're editing
  dashboardState.editingExpenseId = expenseId;
  ...
  if (!categoryId) {
    ...
    const matchingCategory = dashboardState.categories.find(cat => 
  ...
  if (!paymentId) {
    ...
    const matchingPayment = dashboardState.paymentTypes.find(pt => 
  ...
  Swal.fire({
    ...
    html: `
      <div style="text-align:left">
        ...
        <p>💰 Amount: ${CONFIG.utils.formatCurrency(expense.expenseAmount)}</p>
        <p>📅 Date: ${CONFIG.utils.formatDate(expense.expenseDate)}</p>
      </div>
    `,
    icon: 'warning',
...
async function deleteExpense(expenseId) {
  ...
  try {
    ...
    showError("An error occurred while deleting expense");
  }
}

function openExpenseDetailModal(id) {
  const exp = dashboardState.expenses.find(e => String(e.id) === String(id));
  const content = document.getElementById('expenseDetailContent');
  if (!exp) {
    content.innerHTML = '<div class="empty-state">Expense not found</div>';
  } else {
    content.innerHTML = `
      <div class="detail-row"><div class="detail-label">Date:</div><div class="detail-value">${CONFIG.utils.formatDate(exp.expenseDate)}</div></div>
      <div class="detail-row"><div class="detail-label">Amount:</div><div class="detail-value">${CONFIG.utils.formatCurrency(exp.expenseAmount)}</div></div>
...
function openAddExpenseModal() {
  ...
  dashboardState.editingExpenseId = null;
...
?.addEventListener("submit", async (e) => {
  ...
  const isEditing = dashboardState.editingExpenseId !== null;
  ...
  try {
    ...
    console.log(`📤 ${isEditing ? 'Updating' : 'Submitting'} expense:`, expenseData);

    let result;
    if (isEditing) {
      result = await api.updateExpense(dashboardState.editingExpenseId, expenseData);
    } else {
      result = await api.createExpense(expenseData);
    }
    
    console.log(`📥 ${isEditing ? 'Update' : 'Create'} expense result:`, result);

    if (result.success) {
      console.log(`✅ Expense ${isEditing ? 'updated' : 'added'} successfully`);
      showSuccess(`Expense ${isEditing ? 'updated' : 'added'} successfully!`);
      ...
      console.error(`❌ Failed to ${isEditing ? 'update' : 'add'} expense:`, result);
      showError(result.error || `Failed to ${isEditing ? 'update' : 'add'} expense`);
    }
  } catch (error) {
    console.error(`❌ Exception during expense ${isEditing ? 'update' : 'submission'}:`, error);
    showError(`An error occurred while ${isEditing ? 'updating' : 'adding'} expense`);
  } finally {
...
document.getElementById("expenseDetailModal")
?.addEventListener("click", (e) => {
